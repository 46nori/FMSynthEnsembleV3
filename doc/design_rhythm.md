# リズム音源設計仕様（RhythmChannel）

本ドキュメントは、FMSynthEnsembleV3 における **MIDI ch 10（GM パーカッション）** と **リズム音源（YM2608 / YMF288）** の設計を定義する。

**方針**

- ハードウェアは **6 種リズム**（YM2608 / YMF288 共通）のみ。GM の 40 種超のノートは **ソフトウェアのパーカッションマップ** で折り畳む。
...
2. [リズム音源の制約](#2-リズム音源の制約)
3. [モジュール構成と識別](#3-モジュール構成と識別)
4. [複数リズムモジュールの分散ポリシー](#4-複数リズムモジュールの分散ポリシー)
...
| 音源 | **IRhythm フィーチャを持つ YM2608 / YMF288**（`rhythm() != nullptr`） |
...
`MidiFactory` は全 `modules_` から `rhythm() != nullptr` のモジュールを列挙し、`RhythmChannel` コンストラクタが `modules` ベクタを構築する。FM の `NoteVoice` 割当とは独立。
...
MidiFactory(modules_[0..3])
        │
        ▼
RhythmChannel ──► modules[]  … rhythm() フィーチャを持つ OpnBase のみ
        │
        ├── NoteVoice/FM … 各チップ ch0–5 を VoiceAllocator が使用
        └── リズム … チップ単位で RTL/IL/Key を直接操作（Voice なし）
```

- 最大 **4 台**のスロットから、**rhythm フィーチャを持つモジュールの台数 N**（1〜4）を `modules.size()` とする。
- **YM2203** は `rhythm() == nullptr` のため **リズム対象外**。
...
**パーカッションマップ**（[5 章](#5-パーカッションマップ)）は「MIDI ノート番号 → リズム音源の 6 種のどれで鳴らすか」を決める。**排他ノートマップ**（`exclusive_map`）は別の次元で、「**どの MIDI ノート同士が同時には鳴らせないか**」を決める。
...
リズム音源は **HH など 1 スロット 1 系統** しかない。[5 章](#5-パーカッションマップ)のとおり #42 / #44 / #46 は **すべて `RtmInst::HH`** にマップされる。
...
- **1〜5**: 同じ番号のノートは 1 グループ。グループ内で **別 key** の Note On が来たら、前ノートに対応する `RtmInst` を **接続中の全リズムモジュール** で damp してから新規発音する。
