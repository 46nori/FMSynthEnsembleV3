...
6. [信号のタイミング](#信号のタイミング)
7. [YM2203 / YM2608 / YMF288 の差分](#ym2203--ym2608--ymf288-の差分)
8. [YM2203 / YM2608 / YMF288 / 未接続 自動識別アルゴリズム](#ym2203--ym2608--ymf288--未接続-自動識別アルゴリズム)
9. [音量制御](#音量制御)
...
- [YMF288(日本語版)](./datasheet/YMF288_jp.pdf)
...
A1が有効なのはYM2608とYMF288のみ。YM2203ではA1入力を使わず、常にA1=0として扱う。
...
LSI内部では音源部ごとにデータ処理方法が違うため、書き込み終了後から次のモードに移るまでに音源部所定の待ち時間を設定する必要がある。待ち時間(サイクル数)を以下のように定義する。各音源部の実際の値は[書き込み後の待ち時間](#書き込み後の待ち時間)を参照すること。

- **W1**: アドレスライト後の待ち時間
- **W2**: データライト後の待ち時間

アクセス順序は `アドレス指定 → W1 → データライト → W2` の繰り返し。同じアドレスへ連続ライトする場合は、2回目以降のアドレス指定を省略し `データライト → W2` だけを繰り返せばよい。
...
| :-: | --------- | :---------: | ----- |
| 0/1 | 0x21-0xb6 |     17      | FM    |
| 0/1 | 0x00-0x0f |     0 ([＊](#io-portab-書き込みの待ち時間))   | SSG   |
|  0  | 0x10-0x1d |     17      | リズム   |
|  1  | 0x00-0x10 |      0      | ADPCM |
...
### I/O PortA/B 書き込みの待ち時間

上表のとおり、データシート上は I/O PortA/B（A1=0, 0x0e/0x0f）を含む SSG 領域の待ちサイクルは 0 である。しかし実機検証で、0x0e/0x0f への**待ちなしのデータライトを行うと、その直前に書き込んだ FM レジスタの値が正しく反映されなくなる**ことを確認した（全チップ・全 dock で再現。書き込む値の変化・ポートの入出力方向・実ピン駆動の有無・PortA/PortB の別を問わず、データライトのみで発生し、読み出しでは発生しない）。

現象は特定の FM レジスタに限らず、F-Number/Block（音程）・Total Level（音量）・Algorithm/Feedback（音色）・Key on/off（0x28、発音の成否）のいずれでも同じ条件で発現する。既に発音中のチャンネルへ継続的に干渉するのではなく、**FM レジスタへの書き込み直後に I/O ポートへの書き込みを行うタイミングだけ**が危険であり、書き込んだばかりのレジスタ値が正しく反映されない。一方、SSG レジスタ同士（例えば 0x00 と 0x0e の間）の遷移では、一方のレジスタに既知値を書き込んだ直後にもう一方へ低 W1 で書き込み、読み戻して値の変化を確認する総当り検証を行ったが、値が変化することは一切なかった。

FM 相当のアドレスライト後待ち（W1=17）を与えると完全に解消するため、本システムでは 0x0e/0x0f への書き込みに限り W1=17 を適用する。データライト後（W2）は 0 のままで問題ないこと、読み出しは待ち 0 で問題ないことも実機で確認済みであり、必要なのは W1 のみである。

## レジスタ設定のルール

- FM関連レジスタはライトのみ。設定値のリードはできない。リードできるのはステータスレジスタだけ。
- SSG関連レジスタはライト/リード可能。
- ステータスレジスタのBUSY(bit7)は、レジスタへのデータロード中に1になる。YM2608アプリケーションマニュアルによれば、BUSYを監視しながらアクセスする場合はアドレスライト(W1)・データライト(W2)とも待ち時間は不要である。ただしBUSYはFM音源部のレジスタにのみ有効で、リズム部（0x10のW2=576を含む）等には使えない。本システムではフラグ監視は行わず、W1/W2を静的に確保する方式で統一している。
- I/O PortA/B (0x0e/0x0f) の使用条件
	- 入出力方向は0x07のD7(IOB)/D6(IOA)で設定する（0=入力、1=出力）。0x0e/0x0fのリード/ライトに先立ち方向を設定すること。0x07はトーン/ノイズのミキサ設定と同居しているため、ミキサ変更時はリードモディファイライトで方向ビットを保存する
	- 出力モードでは書き込んだデータがホールドされ、次の書き込みまで出力され続ける（イニシャルクリア、または入力モードへの変更で解除）
	- /IC（イニシャルクリア）後、ポートは入力モードになる
	- 出力モードのポート端子に外部から入力電圧を加えてはならない
	- IOA0-7/IOB0-7端子はプルアップ抵抗（60k〜600kΩ）を内蔵する
- オクターブと音程の設定
...
## YM2203 / YM2608 / YMF288 の差分

三チップの機能差分と、CPUバス交直流タイミングの扱いをまとめる。レジスタマップの詳細は前節まで、識別手順は[自動識別アルゴリズム](#ym2203--ym2608--ymf288--未接続-自動識別アルゴリズム)を参照。

### 機能差分

| 項目 | YM2203 (OPN) | YM2608 (OPNA) | YMF288 (OPN3-L) |
| ---- | ------------ | ------------- | --------------- |
| FM | 3ch（4op） | 6ch（4op）。SCH（`0x29`）で CH4–6 有効化 | YM2608 と同様（6ch） |
| リズム | なし | あり（A1=0, `0x10`–`0x1d`） | あり（YM2608 互換） |
| SSG | あり。I/O PortA/B（`0x0e`/`0x0f`）、SSG用DACあり | 同左 | トーン/ノイズ/EG は互換。I/Oポート削除（`0x0e`/`0x0f` 未使用）。SSG専用DACなし |
| ADPCM | なし | あり（A1=1） | なし（関連レジスタ削除） |
| LFO / AMS・PMS / ステレオ LR | なし | あり | あり |
| CSM | あり（`0x27`） | あり | なし（`0x27` D7 未使用） |
| プリスケーラ（`0x2d`–`0x2f`） | あり | あり | なし。分周比は SSG=1/8・FM=1/6・リズム=1/12 固定 |
| A1 | 使わない（常に A1=0） | FM CH4–6・ADPCM で使用 | FM CH4–6 で使用（ADPCM なし） |
| 音声出力 | アナログ（外付けDAC） | アナログ（外付けDAC）＋リズム等 | FM・リズム・SSGをディジタルミックスし、ステレオ16bit（2's complement）を DO から出力。SSG 1ch（Level=15）は FM 1スロット（TL=0）の 0.25 倍 |
| 動作モード | — | — | `/COM` と `0x20` NEW で YM2608B互換 / YMF288（ネイティブ）を切替。本システムはネイティブ前提 |
| 識別コード（`0xFF` リード） | 不定 | `0x01` | ネイティブ `0x02` / 互換 `0x01` |
| その他（YMF288） | — | — | ネイティブ時は全レジスタリード可・スタンバイ可・BUSY/レジスタ待ちが短縮。電源は 5 V / 3.3 V 切替可 |

YMF288 の詳細は[YMF288データシート](./datasheet/YMF288_jp.pdf)の「YM2608Bとの違い」を参照。

### バス・タイミングと安全なアクセス方針

[信号のタイミング](#信号のタイミング) に載せている定格は YM2608（および同等の YM2203）の交直流特性である。YMF288 の CPU バス定格はこれより緩い（短いパルス幅・短いアクセス時間で足りる）。

| 項目（代表値） | YM2203 / YM2608 FM | YM2203 / YM2608 SSG | YMF288 |
| ---- | ---: | ---: | ---: |
| /CS・/WR ライト幅 min | 200 ns | 250 ns | 150 ns |
| ライトデータ setup min | 100 ns | 0 ns | 0 ns |
| /CS・/RD リード幅 min | 250 ns | 400 ns | 200 ns |
| リードアクセス $t_{ACC}$ max | 250 ns（$C_L{=}100\,\mathrm{pF}$） | 400 ns（同） | 200 ns（$C_L{=}50\,\mathrm{pF}$） |
| アドレス setup / hold | 10 / 10 ns | 同 | 同 |

- YM2203 の FM・SSG アクセス定格は YM2608 と同値（[YM2203データシート](./datasheet/YM2203_DataSheet_en.pdf) AC Characteristics）
- YMF288 は FM/SSG を分けず1表。パルス幅・$t_{ACC}$ の要求は YM2608 より短い。ライトデータ setup は 0 ns（YM2608 SSG 寄り）

したがって、**YM2608 のタイミング定格（本ドキュメント「信号のタイミング」の FM・リズム表および SSG 表）を満たせば、YM2203・YM2608・YMF288 のいずれにも安全にアクセスできる。** YMF288 単体ならより短いパルスでも定格内だが、混在駆動では YM2608 側に合わせる。

補足:

- ここでの「タイミング」は交直流特性（パルス幅・setup/hold・$t_{ACC}$ 等）を指す。レジスタ書き込み後の待ちサイクル（W1/W2）は別であり、YMF288 では短縮されている（詳細はデータシート）
- ADPCM は YM2608 専用のため本比較の対象外。ADPCM を使う場合のみ YM2608 の ADPCM 定格を別途満たすこと

## YM2203 / YM2608 / YMF288 / 未接続 自動識別アルゴリズム

- **前提**: YMF288 はネイティブモード（YMF288モード）で動作していること。`/COM=L` かつレジスタ `0x20` の NEW=1。互換モードでは識別コードが YM2608 と同じ `0x01` になり区別できない
- **Step 1**: SSGレジスタへのR/Wで「何らかのOPN系チップ」が存在するか確認
- **Step 2**: レジスタ `0xFF` のリードでチップ種別を判別

### Step 1: チップ接続確認（SSG レジスタ R/W）

YM2203・YM2608・YMF288 すべてに存在するSSGレジスタ（Channel-A Fine Tune: `0x00`）を利用する。
...
SSGのR/W可能レジスタは `0x00`〜`0x0D` および `0x0E`, `0x0F`。`0x00`〜`0x0D` が安全（YMF288 では `0x0E`/`0x0F` は未使用でハイインピーダンス）。
...
3. 結果が 0x01 → YM2608
4. 結果が 0x02 → YMF288（ネイティブモード）
5. それ以外  → YM2203
```

YMF288 データシートではこのリードをステータス2（デバイス識別コード）と呼ぶ。アクセス手順自体は YM2608 の `0xFF` リードと同じ。

### フローチャート

```mermaid
flowchart TD
    Start([スタート]) --> W1["SSG 0x00 に 0xAA を書き込む"]
    W1 --> R1["SSG 0x00 を読み返す"]
    R1 -->|不一致| None1[未接続]
    R1 -->|一致| W2["0x55 で再確認"]
    W2 -->|不一致| None2[未接続]
    W2 -->|一致| ID["レジスタ 0xFF をリード"]
    ID -->|0x01| YM2608[YM2608]
    ID -->|0x02| YMF288[YMF288]
    ID -->|それ以外| YM2203[YM2203]
```
...
| [YMF288データシート](./datasheet/YMF288_jp.pdf) 5-1-3. デバイス識別コード | YM2608B互換モード=`01H`、YMF288モード=`02H`。モードは `/COM` と NEW（`0x20`）で選択 |

### 補足事項

- **書き込み後の待ち**: SSGレジスタはW1/W2とも0のため、書き込み後の追加待ちは不要（BUSYフラグはFM音源部レジスタにのみ有効で、SSGレジスタ書き込みの完了確認には使えない）
- **バス不定値の考慮**: 未接続時にStep 2の判定値が偶然 `0x01` / `0x02` になるリスクを、Step 1のSSG確認で排除している
- **YMF288 互換モード**: `/COM=H` または NEW=0 のとき識別コードは `0x01` となり、本アルゴリズムでは YM2608 と判定される。ネイティブ運用では `/COM` を L に固定し、初期化で `0x20` に NEW=1 を書いてから識別すること
