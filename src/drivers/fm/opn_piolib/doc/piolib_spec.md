...
- `fm_write_reg_raw()`: 投入前に呼ぶ（投入後の idle 待ちは呼ばない。[9.4 節](#94-key-onoff-ゲートレジスタだけ同期させる理由)参照）。
- `write_reg()`: [9.4 節](#94-key-onoff-ゲートレジスタだけ同期させる理由)の対象レジスタに限り、`fm_write_reg_raw()` の直後にもう一度呼ぶ。
...
### 9.4 Key On/Off ゲートレジスタだけ同期させる理由

`fm_write_reg_raw()` は 2 ワード投入後、W2 の消化を待たずに返る。次のバス操作は各 raw プリミティブ先頭の `fm_bus_wait_write_idle()` で SM の完了を待ってから FIFO へ投入するため、理屈上は順序性が保たれる（Issue #19）。

ただし実機では、Stop 時の NoteOff バースト（`$28` FM Key On/Off への連続書き込み）でこの省略が音の止まらない不具合を引き起こすことが確認された（2026-08-15）。診断ビルドの `ENABLE_MIDI_TIMING_STATS` によるキュー滞留・実行時間の計測や VoiceAllocator の成功/失敗数比較では検出できず、実際に曲を止める操作でのみ再現した。原因（SM 側の idle 判定と実チップ内部のビジー状態の食い違いか、他の要因かは未特定）を切り分けられていない。

`$28`（FM Key On/Off）は、周波数・TL・音色・パンのような「次の内部サンプルティックで読まれるだけのパラメータレジスタ」とは異なり、**エンベロープジェネレータの状態遷移そのものをトリガする**レジスタである。このレジスタ（`fm_addr_is_gate_reg()`）に限り、`write_reg()` が `fm_write_reg_raw()` の直後にもう一度 `fm_bus_wait_write_idle()` を呼んで W2 消化を待ってからロックを解放する。それ以外のレジスタは Issue #19 のとおり非同期のまま解放する。

`$10`（A1=0、リズム KeyOn/Damp、W2=576）は `$28` と同じレジスタ種別（状態遷移トリガ）だが、意図的に対象から外している。理由は2つ:

1. 今回の実機不具合の再現条件（曲停止時の Stop）はリズムパートが未使用の区間で発生しており、`$10` 側でも同じ問題が起きるかは未検証
2. NoteVoice の音は明示的な NoteOff まで持続するため、`$28` の書き込みが実効を持たないと「鳴りっぱなし」という分かりやすい形で症状が出る。一方リズムのヒット音は EG で自然減衰する短い発音であり、同種のタイミング不足が `$10` にもあったとしても「音が止まらない」形では現れず、**Damp 効果が薄れる／リトリガが埋もれる**といった地味な音質劣化として出る可能性が高い

`$10` を対象に含めると Issue #19 が最も効果を狙っていたレジスタ（W2=576、他レジスタの数倍）の恩恵を失う。恩恵を優先し、リズムを密に使う曲でのDamp劣化の有無を別途実機で確認する方針とした。**リズムヒット音の再トリガでクリックノイズ・Damp効果の弱まりが確認された場合は、`fm_addr_is_gate_reg()` に `a1 == 0 && addr == 0x10` を戻すこと。**

この切り分けは実機不具合の再現条件（Key On/Off の連続書き込み）から逆算した経験則であり、シリコンレベルの真因（なぜゲートレジスタだけ実機で破綻するか）を説明するものではない。`fm_addr_is_gate_reg()` の対象を減らす、または全レジスタで省略を復活させる変更は、Stop 時の NoteOff バーストのような実際のバーストシナリオで実機検証してから行うこと。
...
I/O PortA/B（A1=0, 0x0e/0x0f）はデータシート上SSG扱い（W1=W2=0）だが、実機ではアドレスライト直後（W1=0）のデータライトを行うと、直前に書き込んだFMレジスタの値（音程・音量・音色・Key on/off等、いずれも同条件で再現）が正しく反映されなくなることが確認されたため、FM相当のW1（17サイクル）を常時適用する。W2は0のままで問題ないこと、読み出しは待ち0のままで問題ないことも実機(YM2608/YM2203)で確認済み。
